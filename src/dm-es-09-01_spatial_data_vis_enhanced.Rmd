...
coordinate system (variable rasterlatlon):

```{r}
rasterlatlon <- projectRaster(
  raster, crs = CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs"))

yat = round(seq(extent(rasterlatlon)@ymin, 
                extent(rasterlatlon)@ymax, length.out = 5), 2)
ylabs = ifelse(yat < 0, paste(yat, "°S", sep = ""),
               ifelse(yat > 0, paste(yat, "°N", sep = ""),
                      paste(yat, "°", sep = "")))

xat = round(seq(extent(rasterlatlon)@xmin, 
                extent(rasterlatlon)@xmax, length.out = 5), 2)
xlabs = ifelse(xat < 0, paste(xat, "°W", sep = ""),
               ifelse(xat > 0, paste(xat, "°E", sep = ""),
                      paste(xat, "°", sep = "")))

spplot(rasterlatlon, col.regions = gray.colors(256), at = breaks,
       panel = function(...){
...
```{r}
setwd("D:/active/moc/dm/examples/data_procd/spatial_data/")
raster_stack <- stack("LC82100502014328LGN00_B3.tif", 
                      "LC82100502014328LGN00_B2.tif", 
                      "LC82100502014328LGN00_B1.tif")

raster_stack[[1]] <- stretch(raster_stack[[1]], minv = 0, maxv = 255, minq = 0.01, maxq = 0.80)
raster_stack[[2]] <- stretch(raster_stack[[2]], minv = 0, maxv = 255, minq = 0.01, maxq = 0.72)
raster_stack[[3]] <- stretch(raster_stack[[3]], minv = 0, maxv = 255, minq = 0.01, maxq = 0.75)
