...
Prior to plotting the heat map, we need to make create a matrix which contains 
the desired information. In our case we want to visualize the number of 
disasters per year per disaster group. To get the data set we can use the table
function applied to columsn year and disaster.group of our original data frame.
The function alligns the groups and years column and row lables and counts the
...
```{r}
data <- table(df$year, df$disaster.group)
head(data)
data <- data^0.5^2

heatmap(data, Rowv = NA, Colv = NA, margins = c(5,5), 
        scale = "column", cexCol=0.75)
```
